...
Prior to Android 6.0 (API level 23), AES key generation was not supported. As a result, many implementations used `SecureRandom` to generate AES keys. However, Android has classes that should be prefered since they take care of certain validations and and quality of the key material.
...
A symmetric encryption key can be derived from a password by using a Password-Based Key Derivation Function such as PBKDF2 which ensures that the derived keys can be used for cryptography purposes. Input parameters for the algorithm are adjusted according to [weak key generation function](0x04g-Testing-Cryptography.md#weak-key-generation-functions) section.

You can derive a strong encryption key based on a password.
